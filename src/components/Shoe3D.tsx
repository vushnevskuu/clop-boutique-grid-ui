...
const Shoe3D = ({ startPosition, velocity, angularVelocity, onRemove }: Shoe3DProps) => {
  ...
  const gravity = -0.015; // Гравитация для дугообразной траектории (применяется к Y) - увеличена для более реалистичного полета
  ...
  // groundY будет вычисляться динамически на основе положения футера
  // Но для упрощения используем 0 как базовый уровень
  const groundY = 0; // Уровень "земли" = 0 (самый низ футера, точка отсчета) - по оси Y
